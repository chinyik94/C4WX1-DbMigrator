...
{
    /// <inheritdoc />
    public partial class AddVwFalangaScores : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"
CREATE OR REPLACE VIEW ""vw_FalangaScores"" AS
WITH FalangaScore AS (
    SELECT 
        ...
        CONCAT(""TC_Auto_Necrosis"", ' %') AS ""BlackEschar"",
        CASE
            WHEN ""TC_Auto_Necrosis"" > 25 THEN 0
            WHEN ""TC_Auto_Necrosis"" > 0 THEN 1
            WHEN ""TC_Auto_Necrosis"" = 0 THEN 2
            ELSE 0
        END AS ""BlackEschar_Score"",
        CASE
            WHEN ""TC_Granulation"" = 0 THEN CONCAT('Granulation = 0% , Depth = ', CONCAT(""SizeDepth"", ' cm'))
            ELSE CONCAT(""SizeDepth"", ' cm')
        END AS ""GreatestWoundDepth"",
        CASE
            WHEN ""TC_Granulation"" = 0 THEN 0
        ...
        CONCAT(""TC_Auto_Epithelizing"", ' %') AS ""PinkWoundBed"",
        CASE
            WHEN ""TC_Auto_Epithelizing"" < 50 THEN 0
            WHEN ""TC_Auto_Epithelizing"" <= 75 THEN 1
            WHEN ""TC_Auto_Epithelizing"" > 75 THEN 2
            ELSE 2
...
");
        ...
        {
            migrationBuilder.Sql(@"
DROP VIEW IF EXISTS ""vw_FalangaScores"";
");
        }
    }
}
